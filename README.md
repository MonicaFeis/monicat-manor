...
1. **Cozy cottage palette** Warm terracotta, honey, and sage tones, paired
   with a hand drawn cottage garden background, reflecting a cottage core
   aesthetic.
...
- Cannot be undone same day (distinct from the permanent favorite)
...
| flake8 (local `.flake8` config) | Python (relaxed PEP8, 99-char limit) | https://flake8.pycqa.org/ |
...
| 28 | `UnorderedObjectListWarning` on the homepage; cats could theoretically shift between pages or be skipped/duplicated across page loads | `SceneView`'s query set uses `.annotate()`, which doesn't reliably preserve the `Cat` model's default `Meta.ordering`, leaving `Paginator` working against an effectively unordered query set | Added an explicit `.order_by('-created_on')` after the `.annotate()` call; covered by a regression test that creates more cats than one page holds and asserts deterministic ordering |
